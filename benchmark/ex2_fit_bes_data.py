...
parser.add_argument('--plot_each', type=bool, default=False)
...
def fit(minos=False):
    ...
    if minos:
        ...
        np.savez(f"full_data_fit_order{Model.pc.order}.npz", fitted_params=fit_result.values, fitted_params_error=fit_result.errors, merrors=minos_errors, cov_matrix=fit_result.covariance,corr_matrix=correlation, accurate=fit_result.accurate, valid=fit_result.valid, fcn_minima=fcn_minima)
    else:
        np.savez(f"full_data_fit_order{Model.pc.order}.npz", fitted_params=fit_result.values, fitted_params_error=fit_result.errors, cov_matrix=fit_result.covariance, corr_matrix=correlation, accurate=fit_result.accurate, valid=fit_result.valid, fcn_minima=fcn_minima)
