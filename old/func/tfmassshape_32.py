...
import math
import numpy as np
_PI = tf.constant(math.pi, dtype=tf.float32)
...
def clip_log_test(x, _epsilon=1e-6):
    """clip log to allowed large value"""
    x_cut = tf.where(x > _epsilon, x, tf.ones_like(x) * _epsilon)
    b_t = tf.math.log(x_cut)
...
def CruijffExtended(m, m0, sigmaL, sigmaR, alphaL, alphaR, beta):
    ...
    f = tf.cast(2.0*sigma*sigma + alpha*dx*dx, tf.float32)
...
def CBShape(m,m0,sigma,alpha,n):
   t =tf.cast((m-m0)/sigma, tf.float32)
   ...
   absAlpha = tf.cast(tf.math.abs(alpha) , tf.float32)
   val_a = tf.cast(tf.math.exp(-0.5*t*t), tf.float32)
   
   a =  tf.cast( tf.math.pow(n/absAlpha,n)*tf.math.exp(-0.5*absAlpha*absAlpha), tf.float32)
   b= tf.cast(n/absAlpha - absAlpha, tf.float32)
   val_b =tf.cast(a/tf.math.pow(b-t, n), tf.float32)
