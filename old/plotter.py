...
for decay in ['b2dk_LL']:#, 'b2dk_DD', 'b2dpi_LL', 'b2dpi_DD']:
    for charge in ['p', 'm']:
        ...
        s12_mc[new_decay] = tf.cast(s12_mc[new_decay][:50000], tf.float64)
        s13_mc[new_decay] = tf.cast(s13_mc[new_decay][:50000], tf.float64)
...
for decay in ['b2dk_LL']:#, 'b2dk_DD', 'b2dpi_LL', 'b2dpi_DD']:
    for charge in ['p', 'm']:
        ...
        eff_comb_a[new_decay] = eff_fun(dalitz_transform(s12_mc[new_decay], s13_mc[new_decay]), 'm', decay.replace('dk', 'dpi'))
        eff_comb_abar[new_decay] = eff_fun(dalitz_transform(s12_mc[new_decay], s13_mc[new_decay]), 'p', decay.replace('dk', 'dpi'))
...
def prod_nll_dk_ll(x, Bsign=1):
    ...
    sig_prob_p = eff[decay+'_p'] * nsig *  prod_totalAmplitudeSquared_XY(1, amp_dk_ll_p, ampbar_dk_ll_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = eff[decay+'_m'] *nsig  * prod_totalAmplitudeSquared_XY(-1, amp_dk_ll_m, ampbar_dk_ll_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_p = eff_misid[decay+'_p'] *nmisid * prod_totalAmplitudeSquared_DPi_XY(1, amp_dk_ll_p, ampbar_dk_ll_p, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_m = eff_misid[decay+'_m'] *nmisid * prod_totalAmplitudeSquared_DPi_XY(-1, amp_dk_ll_m, ampbar_dk_ll_m, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    comb_prob_p = ncomb  * prod_comb(amp_dk_ll_p, ampbar_dk_ll_p, misid_normA_m, misid_normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_p'], eff2=eff_comb_abar[decay+'_p'])
    comb_prob_m = ncomb  * prod_comb(amp_dk_ll_m, ampbar_dk_ll_m, misid_normA_m, misid_normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_m'], eff2=eff_comb_abar[decay+'_m'])
    low_prob_p = eff_comb_abar[decay+'_p'] * nlow * prod_low(1, amp_dk_ll_p, ampbar_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_comb_a[decay+'_m'] * nlow * prod_low(-1, amp_dk_ll_m, ampbar_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')

    low_misID_prob_p =  eff_comb_abar[decay+'_p'] *nlow_misID *  prod_low(1, amp_dk_ll_p, ampbar_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_misID')
    low_misID_prob_m =  eff_comb_a[decay+'_m'] *nlow_misID *  prod_low(-1, amp_dk_ll_m, ampbar_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_misID')
    low_Bs2DKPi_prob_p = eff_comb_a[decay+'_p'] *nlow_Bs2DKPi *  prod_low(-1, amp_dk_ll_p, ampbar_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_Bs2DKPi')
    low_Bs2DKPi_prob_m = eff_comb_abar[decay+'_m'] *nlow_Bs2DKPi *  prod_low(1, amp_dk_ll_m, ampbar_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_Bs2DKPi')

#    sig_prob_p = sig_prob_p/tf.math.reduce_mean(sig_prob_p)
#    sig_prob_m = sig_prob_m/tf.math.reduce_mean(sig_prob_m)
#    misid_prob_p = misid_prob_p/tf.math.reduce_mean(misid_prob_p)
#    misid_prob_m = misid_prob_m/tf.math.reduce_mean(misid_prob_m)
#    comb_prob_p = comb_prob_p/tf.math.reduce_mean(comb_prob_p)
#    comb_prob_m = comb_prob_m/tf.math.reduce_mean(comb_prob_m)
#    low_misID_prob_p = low_misID_prob_p/tf.math.reduce_mean(low_misID_prob_p)
#    low_misID_prob_m = low_misID_prob_m/tf.math.reduce_mean(low_misID_prob_m)
#    low_Bs2DKPi_prob_p = low_Bs2DKPi_prob_p/tf.math.reduce_mean(low_Bs2DKPi_prob_p)
#    low_Bs2DKPi_prob_m = low_Bs2DKPi_prob_m/tf.math.reduce_mean(low_Bs2DKPi_prob_m)
#    low_prob_p = low_prob_p/tf.math.reduce_mean(low_prob_p)
#    low_prob_m = low_prob_m/tf.math.reduce_mean(low_prob_m)
    ...
    prob_p['low_misID'] = (1/total_yield*((low_misID_prob_p/low_normalisation_Bplus)))
    prob_m['low_misID'] = (1/total_yield*((low_misID_prob_m/low_normalisation_Bminus)))
    prob_p['low_Bs2DKPi'] = (1/total_yield*((low_Bs2DKPi_prob_p/low_normalisation_Bminus)))
    prob_m['low_Bs2DKPi'] = (1/total_yield*((low_Bs2DKPi_prob_m/low_normalisation_Bplus)))
...
def plotOn(data=[], var=[], decay='b2dk_ll_p', weights=[], nbins=50, ax=None, range=[5080, 5800], tag=1):
