...
def DeltadeltaD(A, Abar):
    var_a = tf.cast(tf.math.angle(A*np.conj(Abar)), tf.float32) +_PI
...
def totalAmplitudeSquared_Integrated_crossTerm(A, Abar):
    ...
    AAbar = tf.cast(tf.abs(A)*tf.abs(Abar), tf.float32)
    real_part = tf.cast(tf.math.reduce_mean(AAbar*tf.cos(phase)), tf.float32)
    imag_part = tf.cast(tf.math.reduce_mean(AAbar*tf.sin(phase)), tf.float32)
...
def totalAmplitudeSquared_Integrated_new(Bsign=1, A=[], Abar=[], x=(0,0,0,0)):
    ...
    '''
    absA = tf.cast(tf.abs(A), tf.float32)
    absAbar = tf.cast(tf.abs(Abar), tf.float32)
    phase = DeltadeltaD(A, Abar)
    
    if Bsign == 1:
        xPlus = tf.cast(x[0], tf.float32)
        yPlus = tf.cast(x[1], tf.float32)
        rB2 = tf.cast(xPlus**2 + yPlus**2, tf.float32)

        Gp = tf.cast(absA**2 * rB2 + absAbar**2 + 2.0 * (absA * absAbar) *(xPlus * tf.cos(phase) - yPlus * tf.sin(phase)), tf.float32)

        return tf.math.reduce_mean(Gp)
    
    else:
        xMinus = tf.cast(x[2], tf.float32)
        yMinus = tf.cast(x[3], tf.float32)
        rB2 = tf.cast(xMinus**2 + yMinus**2, tf.float32)
        Gm = tf.cast(absA**2 + absAbar**2  * rB2 + 2.0 * (absA * absAbar) *(xMinus * tf.cos(phase) + yMinus * tf.sin(phase)), tf.float32)
...
def totalAmplitudeSquared_Integrated(Bsign=1, normA=1.1, normAbar=1.1, crossTerm=(0, 0), x=(0,0,0,0)):
    ...
    A^2 + Abar^2 * rb^2 + 2*|A||Abar| * rb * cos(deltaB + gamma - deltaD)
    '''
    normA = tf.cast(normA, tf.float32)
    normAbar = tf.cast(normAbar, tf.float32)
    if Bsign == 1:
        xPlus = tf.cast(x[0], tf.float32)
        yPlus = tf.cast(x[1], tf.float32)
        rB2 = tf.cast(xPlus**2 + yPlus**2, tf.float32)

        return tf.cast(normA * rB2 + normAbar + 2.0 *(xPlus *crossTerm[0] - yPlus * crossTerm[1]), tf.float32)
        ...
        return tf.cast(normA + normAbar  * rB2 + 2.0 *(xMinus *crossTerm[0] + yMinus * crossTerm[1]), tf.float32)
...
def totalAmplitudeSquared_DPi_Integrated(Bsign=1, normA=1.1, normAbar=1.1, crossTerm=(0, 0), x=(0,0,0,0)):
    ...
    A^2 + Abar^2 * rb^2 + 2*|A||Abar| * rb * cos(deltaB + gamma - deltaD)
    '''
    xXi = tf.cast(x[4], tf.float32)
    yXi = tf.cast(x[5], tf.float32)
    normA = tf.cast(normA, tf.float32)
    normAbar = tf.cast(normAbar, tf.float32)
    if Bsign == 1:
        xPlus = tf.cast(x[0], tf.float32)
        yPlus = tf.cast(x[1], tf.float32)
        xPlus_DPi = tf.cast(xPlus * xXi - yPlus * yXi, tf.float32)
        yPlus_DPi = tf.cast(yPlus * xXi + xPlus * yXi, tf.float32)

        rB2 = tf.cast(xPlus_DPi**2 + yPlus_DPi**2, tf.float32)
        return tf.cast(normA * rB2 + normAbar + 2.0 *(xPlus_DPi *crossTerm[0] - yPlus_DPi * crossTerm[1]), tf.float32)
    
    else:
        xMinus = tf.cast(x[2], tf.float32)
        yMinus = tf.cast(x[3], tf.float32)
        xMinus_DPi = tf.cast(xMinus * xXi - yMinus * yXi, tf.float32)
        yMinus_DPi = tf.cast(yMinus * xXi + xMinus * yXi, tf.float32)

        rB2 = tf.cast(xMinus_DPi**2 + yMinus_DPi**2, tf.float32)

        return tf.cast(normA + normAbar  * rB2 + 2.0 *(xMinus_DPi *crossTerm[0] + yMinus_DPi * crossTerm[1]), tf.float32)
   
def prod_totalAmplitudeSquared_XY( Bsign=1, amp=[], ampbar=[], x=(0,0,0,0,0,0), pdfs1=[], pdfs2=[], B_M1=[], B_M2=[]):

    phase = DeltadeltaD(amp, ampbar)
    absA = tf.cast(tf.abs(amp), tf.float32)
    absAbar = tf.cast(tf.abs(ampbar), tf.float32)
