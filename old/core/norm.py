...
class Normalisation:
    ...
    def __init__(self, amp_MC, ampbar_MC, name='B2DK'):
        ...
        self._name = name
        self.amp_MC = amp_MC[self._name]
        self.ampbar_MC = ampbar_MC[self._name]
        self._normA = None
        self._normAbar = None
        self._phase = None
    ...
    def phase(self):
        ...
        if self._phase is None:
            self._phase = DeltadeltaD(self.amp_MC, self.ampbar_MC)
    ...
    def normA(self):
        ...
        if self._normA is None:
            self._normA = tf.math.reduce_mean(tf.abs(self.amp_MC)**2)
            self._BacTerms[0] = self._normA
            print('NormA calculated')
    ...
    def normAbar(self):
        ...
        if self._normAbar is None:
            self._normAbar = tf.math.reduce_mean(tf.abs(self.ampbar_MC)**2)
            self._BacTerms[1] = self._normAbar
            print('NormAbar calculated')
    ...
    def AAbar(self):
        ...
        if self._AAbar is None:
            self._AAbar = tf.abs(self.amp_MC) * tf.abs(self.ampbar_MC)
            print('AAbar calculated')
    ...
    def Integrated_crossTerms(self):
        ...
        if self._crossTerms[0] is None or self._crossTerms[1] is None:
            ...
            self._crossTerms[0] = tf.math.reduce_mean(self._AAbar*tf.cos(self._phase))
            self._crossTerms[1] = tf.math.reduce_mean(self._AAbar*tf.sin(self._phase))
            print('CrossTerms:', self._crossTerms)
    ...
    def Integrated_BacTerms(self):
        ...
        if self._BacTerms[0] is None or self._BacTerms[1] is None:
            ...
            print('BacTerms:', self._BacTerms)


        return self._BacTerms
    
    def Integrated(self, Bsign=1, x=(0,0,0,0)):
        '''
    ...
    def Integrated_fullchain(self, Bsign=1, A=[], Abar=[], x=(0,0,0,0)):
    ...
    def Integrated_xi(self, Bsign=1, crossTerm=(0, 0), x=(0,0,0,0)):
        '''
        A^2 * rb^2 + Abar^2 + 2*|A||Abar| * rb * cos(deltaB + gamma + deltaD)
...
class DecayNLLCalculator:
    ...
    def __init__(self, amp_data, ampbar_data, normA, normAbar, normalisationCrossTerms, mass_pdfs, eff_arr):
        self.amp_data = amp_data
        self.ampbar_data = ampbar_data
        self.normA = normA
        self.normAbar = normAbar
        self.normalisationCrossTerms = normalisationCrossTerms
