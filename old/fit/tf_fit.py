...
logpath = '/shared/scratch/pc24403/sim_fit_new_norm'
...
eff_dk_dd_p = np.load(mc_path + '/eff_b2dk_DD_p.npy')
eff_dk_dd_m = np.load(mc_path + '/eff_b2dk_DD_m.npy')
eff_dk_ll_p = np.load(mc_path + '/eff_b2dk_LL_p.npy')
eff_dk_ll_m = np.load(mc_path + '/eff_b2dk_LL_m.npy')
eff_dk_dd_p = tf.constant(eff_dk_dd_p, dtype=tf.float64)
eff_dk_dd_m = tf.constant(eff_dk_dd_m, dtype=tf.float64)
eff_dk_ll_p = tf.constant(eff_dk_ll_p, dtype=tf.float64)
eff_dk_ll_m = tf.constant(eff_dk_ll_m, dtype=tf.float64)
...
eff_dpi_dd_p = np.load(mc_path + '/eff_b2dpi_DD_p.npy')
eff_dpi_dd_m = np.load(mc_path + '/eff_b2dpi_DD_m.npy')
eff_dpi_ll_p = np.load(mc_path + '/eff_b2dpi_LL_p.npy')
eff_dpi_ll_m = np.load(mc_path + '/eff_b2dpi_LL_m.npy')
eff_dpi_dd_p = tf.constant(eff_dpi_dd_p, dtype=tf.float64)
eff_dpi_dd_m = tf.constant(eff_dpi_dd_m, dtype=tf.float64)
eff_dpi_ll_p = tf.constant(eff_dpi_ll_p, dtype=tf.float64)
eff_dpi_ll_m = tf.constant(eff_dpi_ll_m, dtype=tf.float64)
...
def prod_nll_dk_dd(x):
    ...
    normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_dd_p, ampbar_dk_dd_p, eff_dk_dd_p)
    normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_dd_m, ampbar_dk_dd_m, eff_dk_dd_m)
    misid_normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_dd_p, ampbar_dpi_dd_p, eff_dpi_dd_p)
    misid_normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_dd_m, ampbar_dpi_dd_m, eff_dpi_dd_m)

    normA_p = tf.math.reduce_mean(eff_dk_dd_p*tf.math.abs(amp_dk_dd_p)**2)
    normAbar_p = tf.math.reduce_mean(eff_dk_dd_p*tf.math.abs(ampbar_dk_dd_p)**2)
    normA_m = tf.math.reduce_mean(eff_dk_dd_m*tf.math.abs(amp_dk_dd_m)**2)
    normAbar_m = tf.math.reduce_mean(eff_dk_dd_m*tf.math.abs(ampbar_dk_dd_m)**2)

    misid_normA_p = tf.math.reduce_mean(eff_dpi_dd_p*tf.math.abs(amp_dpi_dd_p)**2)
    misid_normAbar_p = tf.math.reduce_mean(eff_dpi_dd_p*tf.math.abs(ampbar_dpi_dd_p)**2)
    misid_normA_m = tf.math.reduce_mean(eff_dpi_dd_m*tf.math.abs(amp_dpi_dd_m)**2)
    misid_normAbar_m = tf.math.reduce_mean(eff_dpi_dd_m*tf.math.abs(ampbar_dpi_dd_m)**2)
...
def prod_nll_dk_ll(x):
    ...
    normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_ll_p, ampbar_dk_ll_p, eff_dk_ll_p)
    normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_ll_m, ampbar_dk_ll_m, eff_dk_ll_m)
    misid_normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_ll_p, ampbar_dpi_ll_p, eff_dpi_ll_p)
    misid_normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_ll_m, ampbar_dpi_ll_m, eff_dpi_ll_m)
    normA_p = tf.math.reduce_mean(eff_dk_ll_p*tf.math.abs(amp_dk_ll_p)**2)
    normAbar_p = tf.math.reduce_mean(eff_dk_ll_p*tf.math.abs(ampbar_dk_ll_p)**2)
    normA_m = tf.math.reduce_mean(eff_dk_ll_m*tf.math.abs(amp_dk_ll_m)**2)
    normAbar_m = tf.math.reduce_mean(eff_dk_ll_m*tf.math.abs(ampbar_dk_ll_m)**2)

    misid_normA_p = tf.math.reduce_mean(eff_dpi_ll_p*tf.math.abs(amp_dpi_ll_p)**2)
    misid_normAbar_p = tf.math.reduce_mean(eff_dpi_ll_p*tf.math.abs(ampbar_dpi_ll_p)**2)
    misid_normA_m = tf.math.reduce_mean(eff_dpi_ll_m*tf.math.abs(amp_dpi_ll_m)**2)
    misid_normAbar_m = tf.math.reduce_mean(eff_dpi_ll_m*tf.math.abs(ampbar_dpi_ll_m)**2)
...
def prod_nll_dpi_dd(x):
    ...
    normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_dd_p, ampbar_dpi_dd_p,eff_dpi_dd_p)
    normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_dd_m, ampbar_dpi_dd_m,eff_dpi_dd_m)
    misid_normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_dd_p, ampbar_dk_dd_p,eff_dk_dd_p)
    misid_normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_dd_m, ampbar_dk_dd_m,eff_dk_dd_m)

    normA_p = tf.math.reduce_mean(eff_dpi_dd_p * tf.math.abs(amp_dpi_dd_p)**2)
    normAbar_p = tf.math.reduce_mean(eff_dpi_dd_p * tf.math.abs(ampbar_dpi_dd_p)**2)
    normA_m = tf.math.reduce_mean(eff_dpi_dd_m * tf.math.abs(amp_dpi_dd_m)**2)
    normAbar_m = tf.math.reduce_mean(eff_dpi_dd_m * tf.math.abs(ampbar_dpi_dd_m)**2)

    misid_normA_p = tf.math.reduce_mean(eff_dk_dd_p *tf.math.abs(amp_dk_dd_p)**2)
    misid_normAbar_p = tf.math.reduce_mean(eff_dk_dd_p *tf.math.abs(ampbar_dk_dd_p)**2)
    misid_normA_m = tf.math.reduce_mean(eff_dk_dd_m *tf.math.abs(amp_dk_dd_m)**2)
    misid_normAbar_m = tf.math.reduce_mean(eff_dk_dd_m *tf.math.abs(ampbar_dk_dd_m)**2)
...
def prod_nll_dpi_ll(x):
    ...
    normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_ll_p, ampbar_dpi_ll_p, eff_dpi_ll_p)
    normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dpi_ll_m, ampbar_dpi_ll_m, eff_dpi_ll_m)
    misid_normalisationCrossTerms_p = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_ll_p, ampbar_dk_ll_p, eff_dk_ll_p)
    misid_normalisationCrossTerms_m = totalAmplitudeSquared_Integrated_crossTerm(amp_dk_ll_m, ampbar_dk_ll_m, eff_dk_ll_m)

    normA_p = tf.math.reduce_mean(eff_dpi_ll_p*tf.math.abs(amp_dpi_ll_p)**2)
    normAbar_p = tf.math.reduce_mean(eff_dpi_ll_p*tf.math.abs(ampbar_dpi_ll_p)**2)
    normA_m = tf.math.reduce_mean(eff_dpi_ll_m*tf.math.abs(amp_dpi_ll_m)**2)
    normAbar_m = tf.math.reduce_mean(eff_dpi_ll_m*tf.math.abs(ampbar_dpi_ll_m)**2)

    misid_normA_p = tf.math.reduce_mean(eff_dk_ll_p*tf.math.abs(amp_dk_ll_p)**2)
    misid_normAbar_p = tf.math.reduce_mean(eff_dk_ll_p*tf.math.abs(ampbar_dk_ll_p)**2)
    misid_normA_m = tf.math.reduce_mean(eff_dk_ll_m*tf.math.abs(amp_dk_ll_m)**2)
    misid_normAbar_m = tf.math.reduce_mean(eff_dk_ll_m*tf.math.abs(ampbar_dk_ll_m)**2)
