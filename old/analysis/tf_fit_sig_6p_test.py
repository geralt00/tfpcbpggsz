...
logpath = '/dice/users/pc24403/BPGGSZ/sig_only_6p'
...
def prod_nll_dk_dd(x):
    ...
    eff_p = eff_fun(dalitz_transform(s12_data[decay+'_p'], s13_data[decay+'_p']), 'p', decay)
    eff_m = eff_fun(dalitz_transform(s12_data[decay+'_m'], s13_data[decay+'_m']), 'm', decay)
    eff_p = tf.cast(eff_p, tf.float64)
    eff_m = tf.cast(eff_m, tf.float64)

    ll_data_p = clip_log(eff_p*sig_prob_p/normalisation_Bplus)
    ll_data_m = clip_log(eff_m*sig_prob_m/normalisation_Bminus)
...
def prod_nll_dpi_dd(x):
    ...
    sig_prob_p = prod_totalAmplitudeSquared_DPi_XY(1, amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = prod_totalAmplitudeSquared_DPi_XY(-1, amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
...
def prod_nll_dpi_ll(x):
    ...
    sig_prob_p = prod_totalAmplitudeSquared_DPi_XY(1, amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = prod_totalAmplitudeSquared_DPi_XY(-1, amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
