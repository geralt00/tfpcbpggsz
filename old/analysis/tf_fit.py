...
logpath = '/dice/users/pc24403/BPGGSZ/sim_fit_1x'
...
def prod_nll_dpi_dd(x):
    ...
    comb_normalisation_Bplus = tf.cast((normA_p + normAbar_m)*0.5*fracDD + (1-fracDD), tf.float64)
    comb_normalisation_Bminus = tf.cast((normA_p + normAbar_m)*0.5*fracDD + (1-fracDD), tf.float64)

    low_normalisation_Bplus = tf.cast(normAbar_m, tf.float64)
    low_normalisation_Bminus = tf.cast(normA_p, tf.float64)
