...
def get_p4(decay="b2dpi", cut='', index=index):
    ...
    if cut == 'int':
        ...
        branch_names = ["_1_K0S0_E", "_1_K0S0_Px", "_1_K0S0_Py", "_1_K0S0_Pz",
                         ...
                         "_3_pi~_E", "_3_pi~_Px", "_3_pi~_Py", "_3_pi~_Pz"]
        if cut == 'p':
            file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/1x_test/phsp/{decay}_test_{index}.root:Bplus_DalitzEventList'
#            file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/swap/{decay}_{comp}_{index}.root:Bplus_DalitzEventList'

        else:
            file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/1x_test/phsp/{decay}_test_{index}.root:Bminus_DalitzEventList'
...
if update == True:
    ...
    for decay in ['DK_KsPiPi_LL', 'DK_KsPiPi_DD', 'DPi_KsPiPi_LL', 'DPi_KsPiPi_DD']:
        ...
        file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/1x_test/phsp/{new_decay}_test_{index}.root:Bplus_DalitzEventList'
        tree = up.open(file_name)
        for comp in comp_tag:
            array = tree.arrays('tagmode','(B_M>5080) & (tagmode==%d)'%comp_tag[comp])
            varDict[f'n_{comp}_{decay}'] = 2*len(array)
            print(f'n_{comp}_{decay} = {2*len(array)}')
...
logpath = '/dice/users/pc24403/BPGGSZ/sim_fit_phsp_comb'
...
def prod_nll_dk_dd(x):
    ...
    comb_prob_p =  ncomb * mass_pdfs[decay+'_p']['comb'] * prod_comb(amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, misid_normA_m, misid_normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    comb_prob_m = ncomb * mass_pdfs[decay+'_m']['comb'] * prod_comb(amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, misid_normA_m, misid_normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    low_prob_p = eff_low[decay+'_p'] * nlow * mass_pdfs[decay+'_p']['low'] * prod_low(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_low[decay+'_m'] * nlow * mass_pdfs[decay+'_m']['low'] * prod_low(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
    low_misID_prob_p =  eff_low[decay+'_p'] * nlow_misID * mass_pdfs[decay+'_p']['low_misID'] * prod_low(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_misID')
    low_misID_prob_m =  eff_low[decay+'_m'] * nlow_misID * mass_pdfs[decay+'_m']['low_misID'] * prod_low(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_misID')
    low_Bs2DKPi_prob_p = eff_low[decay+'_p'] * nlow_Bs2DKPi * mass_pdfs[decay+'_p']['low_Bs2DKPi'] * prod_low(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_Bs2DKPi')
    low_Bs2DKPi_prob_m = eff_low[decay+'_m'] * nlow_Bs2DKPi * mass_pdfs[decay+'_m']['low_Bs2DKPi'] * prod_low(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_Bs2DKPi')
    ...
    ll_data_p = clip_log((sig_prob_p/normalisation_Bplus) + (misid_prob_p/misid_normalisation_Bplus)  + (low_prob_p/low_normalisation_Bplus) + (low_misID_prob_p/low_normalisation_Bplus) + (low_Bs2DKPi_prob_p/low_normalisation_Bplus) + (comb_prob_p))
    ll_data_m = clip_log((sig_prob_m/normalisation_Bminus) + (misid_prob_m/misid_normalisation_Bminus) + (low_prob_m/low_normalisation_Bminus) + (low_misID_prob_m/low_normalisation_Bminus) + (low_Bs2DKPi_prob_m/low_normalisation_Bminus) + (comb_prob_m))
...
def prod_nll_dk_ll(x):
    ...
    comb_prob_p = ncomb * mass_pdfs[decay+'_p']['comb'] * prod_comb(amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, misid_normA_m, misid_normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    comb_prob_m = ncomb * mass_pdfs[decay+'_m']['comb'] * prod_comb(amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, misid_normA_m, misid_normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    low_prob_p = eff_low[decay+'_p'] * nlow * mass_pdfs[decay+'_p']['low'] * prod_low(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_low[decay+'_m'] * nlow * mass_pdfs[decay+'_m']['low'] * prod_low(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
    low_misID_prob_p =  eff_low[decay+'_p'] *nlow_misID * mass_pdfs[decay+'_p']['low_misID'] * prod_low(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_misID')
    low_misID_prob_m =  eff_low[decay+'_m'] *nlow_misID * mass_pdfs[decay+'_m']['low_misID'] * prod_low(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_misID')
    low_Bs2DKPi_prob_p = eff_low[decay+'_p'] *nlow_Bs2DKPi * mass_pdfs[decay+'_p']['low_Bs2DKPi'] * prod_low(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_Bs2DKPi')
    low_Bs2DKPi_prob_m = eff_low[decay+'_m'] *nlow_Bs2DKPi * mass_pdfs[decay+'_m']['low_Bs2DKPi'] * prod_low(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_Bs2DKPi')
    ...
    misid_normalisation_Bplus = totalAmplitudeSquared_DPi_Integrated(1, misid_normA_p, misid_normAbar_p, misid_normalisationCrossTerms_p, x)
    misid_normalisation_Bminus = totalAmplitudeSquared_DPi_Integrated(-1, misid_normA_m, misid_normAbar_m, misid_normalisationCrossTerms_m, x) 
    comb_normalisarion = tf.cast((misid_normA_m+misid_normAbar_p)*0.5*fracDD + (1.0-fracDD), tf.float64)
    low_normalisation_Bplus = tf.cast(misid_normAbar_p, tf.float64)
    low_normalisation_Bminus = tf.cast(misid_normA_m, tf.float64)
...
def prod_nll_dpi_dd(x):
    ...
    comb_prob_p = ncomb * mass_pdfs[decay+'_p']['comb'] * prod_comb(amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, normA_m, normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    comb_prob_m = ncomb * mass_pdfs[decay+'_m']['comb'] * prod_comb(amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, normA_m, normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    low_prob_p = eff_low[decay+'_p'] * nlow * mass_pdfs[decay+'_p']['low'] * prod_low(1, amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_low[decay+'_m'] * nlow * mass_pdfs[decay+'_m']['low'] * prod_low(-1, amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
...
def prod_nll_dpi_ll(x):
    ...
    comb_prob_p = ncomb * mass_pdfs[decay+'_p']['comb'] * prod_comb(amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, normA_m, normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    comb_prob_m = ncomb * mass_pdfs[decay+'_m']['comb'] * prod_comb(amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, normA_m, normAbar_p, fracDD=fracDD,eff1=eff_low[decay+'_m'], eff2=eff_low[decay+'_p'])
    low_prob_p = eff_low[decay+'_p'] * nlow * mass_pdfs[decay+'_p']['low'] * prod_low(1, amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_low[decay+'_m'] * nlow * mass_pdfs[decay+'_m']['low'] * prod_low(-1, amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
