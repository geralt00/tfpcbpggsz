...
if update == True:
    ...
    for decay in ['DK_KsPiPi_LL', 'DK_KsPiPi_DD', 'DPi_KsPiPi_LL', 'DPi_KsPiPi_DD']:
        ...
        for comp in comp_tag:
            ...
            print(f'n_{comp}_{decay} = {len(array)}')
...
logpath = '/dice/users/pc24403/BPGGSZ/sim_fit_dpi_without_comb'
...
def prod_nll_dk_dd(x):
    ...
    ll_data_p = clip_log((sig_prob_p/normalisation_Bplus) + (misid_prob_p/misid_normalisation_Bplus)  + (low_prob_p/low_normalisation_Bplus) + (low_misID_prob_p/low_normalisation_Bplus) + (low_Bs2DKPi_prob_p/low_normalisation_Bplus) + (comb_prob_p/comb_normalisarion))
    ll_data_m = clip_log((sig_prob_m/normalisation_Bminus) + (misid_prob_m/misid_normalisation_Bminus) + (low_prob_m/low_normalisation_Bminus) + (low_misID_prob_m/low_normalisation_Bminus) + (low_Bs2DKPi_prob_m/low_normalisation_Bminus) + (comb_prob_m/comb_normalisarion))
...
def prod_nll_dpi_dd(x):
    ...
    comb_prob_p = ncomb * mass_pdfs[decay+'_p']['comb'] * prod_comb(amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, normA_m, normAbar_p, fracDD=fracDD)
    comb_prob_m = ncomb * mass_pdfs[decay+'_m']['comb'] * prod_comb(amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, normA_m, normAbar_p, fracDD=fracDD)
    ...
    ll_data_p = clip_log((sig_prob_p/normalisation_Bplus) + (misid_prob_p/misid_normalisation_Bplus) +  (low_prob_p/low_normalisation_Bplus))
    ll_data_m = clip_log((sig_prob_m/normalisation_Bminus) + (misid_prob_m/misid_normalisation_Bminus) +  (low_prob_m/low_normalisation_Bminus))
