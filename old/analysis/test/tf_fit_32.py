...
for decay in ['b2dk_LL', 'b2dk_DD', 'b2dpi_LL', 'b2dpi_DD']:
    for charge in ['p', 'm']:
        ...
        for comp in comps:
            ...
            mass_pdfs[new_decay][comp] = tf.cast(pdfs_data[new_decay][comp](Bu_M[new_decay]), tf.float32)
time2 = time.time()
...
for decay in ['b2dk_LL', 'b2dk_DD', 'b2dpi_LL', 'b2dpi_DD']:
    for charge in ['p', 'm']:
        new_decay = decay + '_'+ charge
        eff[new_decay] = tf.cast(eff_fun(dalitz_transform(s12_data[new_decay], s13_data[new_decay]), charge, decay), tf.float32)
        eff_low[new_decay] = tf.cast(eff_fun(dalitz_transform(s12_data[new_decay], s13_data[new_decay]), charge, decay.replace('dk', 'dpi')), tf.float32)
        eff_comb_a[new_decay] = tf.cast(eff_fun(dalitz_transform(s12_data[new_decay], s13_data[new_decay]), 'm', decay.replace('dk', 'dpi')), tf.float32)
        eff_comb_abar[new_decay] = tf.cast(eff_fun(dalitz_transform(s12_data[new_decay], s13_data[new_decay]), 'p', decay.replace('dk', 'dpi')), tf.float32)
        if decay.split('_')[0] == 'b2dpi':
            eff_misid[new_decay] = tf.cast(eff_fun(dalitz_transform(s12_data[new_decay], s13_data[new_decay]), charge, decay.replace('dpi', 'dk')), tf.float32)
        elif decay.split('_')[0] == 'b2dk':
            eff_misid[new_decay] = tf.cast(eff_fun(dalitz_transform(s12_data[new_decay], s13_data[new_decay]), charge, decay.replace('dk', 'dpi')), tf.float32)
...
if update == True:
    ...
    for decay in ['DK_KsPiPi_LL', 'DK_KsPiPi_DD', 'DPi_KsPiPi_LL', 'DPi_KsPiPi_DD']:
        ...
        for comp in comp_tag:
            ...
            varDict[f'n_{comp}_{decay}'] = tf.cast(len(array), tf.float32)
    ...
    for decay in ['b2dk_LL', 'b2dk_DD', 'b2dpi_LL', 'b2dpi_DD']:
        ...
        frac_DD_dic[decay] = tf.cast(2*len(array_dd)/(2*len(array_dd)+len(array_flat)), tf.float32)
...
#    frac_low_misID = varDict['n_low_misID_DK_KsPiPi_DD']/varDict['n_low_DPi_KsPiPi_DD']
    ...
    low_normalisation_Bplus = tf.cast(misid_normAbar_p, tf.float32)
    low_normalisation_Bminus = tf.cast(misid_normA_m, tf.float32)
