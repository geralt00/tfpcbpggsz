...
for decay in ['b2dk_LL', 'b2dk_DD', 'b2dpi_LL', 'b2dpi_DD']:
    file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/1x_test/{decay}_comb_{index}.root:Bplus_DalitzEventList'
    tree = up.open(file_name)
...
def prod_nll_dk_dd(x):
    ...
    sig_prob_p = eff[decay+'_p']* prod_totalAmplitudeSquared_XY(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = eff[decay+'_m']* prod_totalAmplitudeSquared_XY(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_p = eff_misid[decay+'_p'] * nmisid * prod_totalAmplitudeSquared_DPi_XY(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_m = eff_misid[decay+'_m'] * nmisid * prod_totalAmplitudeSquared_DPi_XY(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
#    comb_prob_p = ncomb * prod_comb(amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, misid_normA_m, misid_normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_p'], eff2=eff_comb_abar[decay+'_p'])
#    comb_prob_m = ncomb * prod_comb(amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, misid_normA_m, misid_normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_m'], eff2=eff_comb_abar[decay+'_m'])
    low_prob_p = eff_comb_abar[decay+'_p'] * nlow * prod_low(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_comb_a[decay+'_m'] * nlow * prod_low(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
    low_misID_prob_p =  eff_comb_abar[decay+'_p'] * nlow_misID * prod_low(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_misID')
    low_misID_prob_m =  eff_comb_a[decay+'_m'] * nlow_misID  * prod_low(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_misID')
    low_Bs2DKPi_prob_p = eff_comb_a[decay+'_p'] * nlow_Bs2DKPi  * prod_low(-1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_Bs2DKPi')
    low_Bs2DKPi_prob_m = eff_comb_abar[decay+'_m'] * nlow_Bs2DKPi * prod_low(1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_Bs2DKPi')
    comb_prob_p = eff_comb_a[decay+'_p']*fracDD*0.5*tf.math.abs(amp_Data_dk_dd_p)**2/misid_normA_m + eff_comb_abar[decay+'_p']*fracDD*0.5*tf.math.abs(ampbar_Data_dk_dd_p)**2/misid_normAbar_p + (1-fracDD)*tf.ones_like(tf.math.abs(ampbar_Data_dk_dd_p)**2)
    comb_prob_m = eff_comb_a[decay+'_m']*fracDD*0.5*tf.math.abs(amp_Data_dk_dd_m)**2/misid_normA_m + eff_comb_abar[decay+'_m']*fracDD*0.5*tf.math.abs(ampbar_Data_dk_dd_m)**2/misid_normAbar_p + (1-fracDD)*tf.ones_like(tf.math.abs(ampbar_Data_dk_dd_m)**2)
...
#    ll_data_m = clip_log((sig_prob_m/normalisation_Bminus) + (misid_prob_m/misid_normalisation_Bminus) + (low_prob_m/low_normalisation_Bminus) + (low_misID_prob_m/low_normalisation_Bminus) + (low_Bs2DKPi_prob_m/low_normalisation_Bplus) + (comb_prob_m))
    ll_data_p = clip_log((sig_prob_p/normalisation_Bplus)*nsig/(nsig+ncomb)   + (comb_prob_p)*ncomb/(nsig+ncomb))
    ll_data_m = clip_log((sig_prob_m/normalisation_Bminus)*nsig/(nsig+ncomb)  + (comb_prob_m)*ncomb/(nsig+ncomb))


    ext_nll = (tf.reduce_sum( -2* ll_data_p) + tf.reduce_sum( -2*ll_data_m) + 2 * total_yield)
    nll = tf.reduce_sum( -2* (ll_data_p + ll_data_m)  )
    return nll
...
def prod_nll_dk_ll(x):
    ...
    sig_prob_p = eff[decay+'_p'] * prod_totalAmplitudeSquared_XY(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = eff[decay+'_m'] * prod_totalAmplitudeSquared_XY(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_p = eff_misid[decay+'_p'] *nmisid * prod_totalAmplitudeSquared_DPi_XY(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_m = eff_misid[decay+'_m'] *nmisid * prod_totalAmplitudeSquared_DPi_XY(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    #comb_prob_p = ncomb * prod_comb(amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, misid_normA_m, misid_normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_p'], eff2=eff_comb_abar[decay+'_p'])
    #comb_prob_m = ncomb * prod_comb(amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, misid_normA_m, misid_normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_m'], eff2=eff_comb_abar[decay+'_m'])
    low_prob_p = eff_comb_abar[decay+'_p'] * nlow * prod_low(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_comb_a[decay+'_m'] * nlow * prod_low(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
    low_misID_prob_p =  eff_comb_abar[decay+'_p'] *nlow_misID * prod_low(1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_misID')
    low_misID_prob_m =  eff_comb_a[decay+'_m'] *nlow_misID  * prod_low(-1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_misID')
    low_Bs2DKPi_prob_p = eff_comb_a[decay+'_p'] *nlow_Bs2DKPi  * prod_low(-1, amp_Data_dk_ll_p, ampbar_Data_dk_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low_Bs2DKPi')
    low_Bs2DKPi_prob_m = eff_comb_abar[decay+'_m'] *nlow_Bs2DKPi * prod_low(1, amp_Data_dk_ll_m, ampbar_Data_dk_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low_Bs2DKPi')
    comb_prob_p = eff_comb_a[decay+'_p']*fracDD*0.5*tf.math.abs(amp_Data_dk_ll_p)**2/misid_normA_m + eff_comb_abar[decay+'_p']*fracDD*0.5*tf.math.abs(ampbar_Data_dk_ll_p)**2/misid_normAbar_p + (1-fracDD)*tf.ones_like(tf.math.abs(ampbar_Data_dk_ll_p)**2)
    comb_prob_m = eff_comb_a[decay+'_m']*fracDD*0.5*tf.math.abs(amp_Data_dk_ll_m)**2/misid_normA_m + eff_comb_abar[decay+'_m']*fracDD*0.5*tf.math.abs(ampbar_Data_dk_ll_m)**2/misid_normAbar_p + (1-fracDD)*tf.ones_like(tf.math.abs(ampbar_Data_dk_ll_m)**2)
    total_yield = (nsig + nmisid + ncomb + nlow + nlow_misID + nlow_Bs2DKPi)*2
...
def prod_nll_dpi_dd(x):
    ...
    sig_prob_p = eff[decay+'_p'] * prod_totalAmplitudeSquared_DPi_XY(1, amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = eff[decay+'_m'] * prod_totalAmplitudeSquared_DPi_XY(-1, amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_p = eff_misid[decay+'_p'] * nmisid * prod_totalAmplitudeSquared_XY(1, amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_m = eff_misid[decay+'_m'] * nmisid * prod_totalAmplitudeSquared_XY(-1, amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    comb_prob_p = ncomb * prod_comb(amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, normA_m, normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_p'], eff2=eff_comb_abar[decay+'_p'])
    comb_prob_m = ncomb * prod_comb(amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, normA_m, normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_m'], eff2=eff_comb_abar[decay+'_m'])
    low_prob_p = eff_comb_abar[decay+'_p'] * nlow * prod_low(1, amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_comb_a[decay+'_m'] * nlow * prod_low(-1, amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')
...
def prod_nll_dpi_ll(x):
    ...
    sig_prob_p = eff[decay+'_p'] * nsig * prod_totalAmplitudeSquared_DPi_XY(1, amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = eff[decay+'_m'] * nsig * prod_totalAmplitudeSquared_DPi_XY(-1, amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_p = eff_misid[decay+'_p'] * nmisid * prod_totalAmplitudeSquared_XY(1, amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    misid_prob_m = eff_misid[decay+'_m'] * nmisid * prod_totalAmplitudeSquared_XY(-1, amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, x, mass_pdfs[decay+'_p']['misid'], mass_pdfs[decay+'_m']['misid'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    comb_prob_p = ncomb * prod_comb(amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, normA_m, normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_p'], eff2=eff_comb_abar[decay+'_p'])
    comb_prob_m = ncomb * prod_comb(amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, normA_m, normAbar_p, fracDD=fracDD, eff1=eff_comb_a[decay+'_m'], eff2=eff_comb_abar[decay+'_m'])
    low_prob_p = eff_comb_abar[decay+'_p'] * nlow * prod_low(1, amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, x, mass_pdfs[decay+'_p'], Bu_M[decay+'_p'], type='low')
    low_prob_m = eff_comb_a[decay+'_m'] * nlow * prod_low(-1, amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, x, mass_pdfs[decay+'_m'], Bu_M[decay+'_m'], type='low')


    total_yield = (nsig + nmisid + ncomb + nlow)
...
def nll_dpi_dd(x):
    x1 = x[:12]
    x2 = tf.concat([x[:6], x[12:18]],0)
    return prod_nll_dpi_dd(x1)

def nll_dpi_ll(x):
    x1 = x[:12]
    x2 = tf.concat([x[:6], x[12:18]],0)
    return prod_nll_dpi_ll(x2)

def nll_dpi(x):
    x1 = x[:12]
    x2 = tf.concat([x[:6], x[12:18]],0)
    return prod_nll_dpi_dd(x1) + prod_nll_dpi_ll(x2)
...
m.fixed = [False, False, False, False, True, True, True, True, True, True, True, True, True, True, True, True, True, True]
mg = m.migrad()
#mg = m.minos()
