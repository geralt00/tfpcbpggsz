...
def rnd_int(arr: np.array, size: int = 1) -> np.array:
    rng = np.random.default_rng()
    return arr[0, rng.choice(arr.shape[1], size=size, replace=False)]
...
def get_p4(decay="b2dpi", cut='', index=index):
    ...
    if cut == 'int':
        ...
        branch_names = ["_1_K0S0_E", "_1_K0S0_Px", "_1_K0S0_Py", "_1_K0S0_Pz",
                         ...
                         "_3_pi~_E", "_3_pi~_Px", "_3_pi~_Py", "_3_pi~_Pz"]
        if cut == 'p':
            file_name = f'/dice/users/pc24403/PCBPGGSZ/fromLZU/bootstrap/b2dk_DD_sig_{index}.root:Bplus_DalitzEventList'
#            file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/swap/pullData_{comp}_{index}.root:Bplus__DalitzEventList'

        else:
            file_name = f'/dice/users/pc24403/PCBPGGSZ/fromLZU/bootstrap/b2dk_DD_sig_{index}.root:Bminus_DalitzEventList'
#            file_name = f'/software/pc24403/PCBPGGSZ/outputs/toy/swap/pullData_{comp}_{index}.root:Bminus__DalitzEventList'
...
def getAmp(decay='b2dpi', cut='int', index=index):
    ...
    return amplitude, np.negative(amplitudeBar)
...
logpath = '/dice/users/pc24403/BPGGSZ/sig_only_fit_no_eff'
...
ampbar_dk_dd_p = ampbar_dk_dd_p[0,seed*500000:(seed+1)*500000]
amp_dk_dd_m = np.load(mc_path + '/Int_b2dk_DD_m_amp.npy')
amp_dk_dd_m = amp_dk_dd_m[0,seed*500000:(seed+1)*500000]
ampbar_dk_dd_m = np.load(mc_path + '/Int_b2dk_DD_m_ampbar.npy')
ampbar_dk_dd_m = ampbar_dk_dd_m[0,seed*500000:(seed+1)*500000]
...
def prod_nll_dk_dd(x):
    ...
    normAbar_p = tf.math.reduce_mean(tf.math.abs(amp_dk_dd_p)**2)
    normA_m = tf.math.reduce_mean(tf.math.abs(amp_dk_dd_m)**2)
    normAbar_m = tf.math.reduce_mean(tf.math.abs(ampbar_dk_dd_m)**2)



    sig_prob_p = prod_totalAmplitudeSquared_XY(1, amp_Data_dk_dd_p, ampbar_Data_dk_dd_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = prod_totalAmplitudeSquared_XY(-1, amp_Data_dk_dd_m, ampbar_Data_dk_dd_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
...
#    normalisation_Bplus = totalAmplitudeSquared_Integrated_new(1, amp_dk_dd_p, ampbar_dk_dd_p, x)
#    normalisation_Bminus = totalAmplitudeSquared_Integrated_new(-1, amp_dk_dd_m, ampbar_dk_dd_m, x)
   


    ll_data_p = clip_log(sig_prob_p/normalisation_Bplus)
    ll_data_m = clip_log(sig_prob_m/normalisation_Bminus)
...
def prod_nll_dpi_dd(x):
    ...
    sig_prob_p = prod_totalAmplitudeSquared_XY(1, amp_Data_dpi_dd_p, ampbar_Data_dpi_dd_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = prod_totalAmplitudeSquared_XY(-1, amp_Data_dpi_dd_m, ampbar_Data_dpi_dd_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
...
def prod_nll_dpi_ll(x):
    ...
    sig_prob_p = prod_totalAmplitudeSquared_XY(1, amp_Data_dpi_ll_p, ampbar_Data_dpi_ll_p, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
    sig_prob_m = prod_totalAmplitudeSquared_XY(-1, amp_Data_dpi_ll_m, ampbar_Data_dpi_ll_m, x, mass_pdfs[decay+'_p']['sig'], mass_pdfs[decay+'_m']['sig'], Bu_M[decay+'_p'], Bu_M[decay+'_m'])
...
#x = np.zeros(18)
m_dk = iminuit.Minuit(nll_dk_dd, x)
#m_dk.limits = [None, None, None, None, None, None, (-n_data['n_dk_dd'], n_data['n_dk_dd']), (-n_data['n_dk_dd'], n_data['n_dk_dd']), (-n_data['n_dk_dd'], n_data['n_dk_dd']), (-n_data['n_dpi_dd'], n_data['n_dpi_dd']), (-n_data['n_dpi_dd'], n_data['n_dpi_dd']), (-n_data['n_dpi_dd'], n_data['n_dpi_dd']), (-n_data['n_dk_ll'], n_data['n_dk_ll']), (-n_data['n_dk_ll'], n_data['n_dk_ll']), (-n_data['n_dk_ll'], n_data['n_dk_ll']), (-n_data['n_dpi_ll'], n_data['n_dpi_ll']), (-n_data['n_dpi_ll'], n_data['n_dpi_ll']), (-n_data['n_dpi_ll'], n_data['n_dpi_ll'])]
m_dk.fixed = [False, False, False, False, True, True, True, True, True, True, True, True, True, True, True, True, True, True]
mg_dk = m_dk.migrad()


with open(f'{logpath}/b2dk_output_{index}.txt', 'w') as f:
    print(mg_dk, file=f)
    print(m_dk.values, file=f)
    print(m_dk.errors, file=f)
    print(m_dk.fval, file=f)
    print(m_dk.nfcn, file=f)
    print(m_dk.covariance, file=f)

#m_dpi = iminuit.Minuit(nll_dpi, x)
#m_dpi.limits = [None, None, None, None, None, None, (-n_data['n_dk_dd'], n_data['n_dk_dd']), (-n_data['n_dk_dd'], n_data['n_dk_dd']), (-n_data['n_dk_dd'], n_data['n_dk_dd']), (-n_data['n_dpi_dd'], n_data['n_dpi_dd']), (-n_data['n_dpi_dd'], n_data['n_dpi_dd']), (-n_data['n_dpi_dd'], n_data['n_dpi_dd']), (-n_data['n_dk_ll'], n_data['n_dk_ll']), (-n_data['n_dk_ll'], n_data['n_dk_ll']), (-n_data['n_dk_ll'], n_data['n_dk_ll']), (-n_data['n_dpi_ll'], n_data['n_dpi_ll']), (-n_data['n_dpi_ll'], n_data['n_dpi_ll']), (-n_data['n_dpi_ll'], n_data['n_dpi_ll'])]
#m_dpi.fixed = [False, False, False, False, True, True, True, True, True, True, True, True, True, True, True, True, True, True]
#mg_dpi = m_dpi.migrad()
#
#
#with open(f'{logpath}/b2dpi_output_{index}.txt', 'w') as f:
#    print(mg_dpi, file=f)
#    print(m_dpi.values, file=f)
#    print(m_dpi.errors, file=f)
#    print(m_dpi.fval, file=f)
#    print(m_dpi.nfcn, file=f)
#    print(m_dpi.covariance, file=f)
