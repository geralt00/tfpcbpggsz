...
def fit_result(file_path, is_polar=False):
    ...
    variables_to_search = ['xPlus', 'xMinus', 'yPlus', 'yMinus', 'xXi', 'yXi']
    fit_results = []
...
def run_fit(order=1,type="_"):
    ...
    input_ckm = {'xPlus': -0.0897, 'xMinus': 0.0586, 'yPlus': -0.0110, 'yMinus': 0.0688, 'xXi': -0.0549, 'yXi': 0.0070}
    title_latex = {'xPlus': 'x^{DK}_{+}', 'xMinus': 'x^{DK}_{-}', 'yPlus': 'y^{DK}_{+}', 'yMinus': 'y^{DK}_{-}', 'xXi': 'x^{D#pi}_{#xi}', 'yXi': 'y^{D#pi}_{#xi}'}
    # Define variable names and their corresponding RooRealVar objects
    variable_names = ['xPlus', 'xMinus', 'yPlus', 'yMinus', 'xXi', 'yXi']
    variables = {}
    mean_var = {}
    sigma_var = {}
    ...
    for index in range(1, nsets+1):
        full_str=""
        file_paths.append(f"/shared/scratch/pc24403/cpfit/log/simfit_eff_{index}.log")
