    ...
    for bdecay in Bdecays:
        for type in Types:
            for charge in Charges:
                ...
                decay = f'{bdecay}_{type}'
                s12_data, s13_data = getMass(decay+'_'+charge, cut='int')
                eff=[]
                print('INFO: Calculate efficiency...')
                eff = eff_fun(dalitz_transform(s12_data, s13_data), charge, decay)
                print('INFO: mass Function loaded...')
                data_path = '/shared/scratch/pc24403/amp_ampgen'
                os.makedirs(data_path, exist_ok=True)
                np.save(f'{data_path}/eff_{decay}_{charge}.npy', eff)
                end = time.time()
