...
class Ntuple:
    ...
    def get_preliminary_cuts_eff(self):
        ...
        for year_item in self.year.list_years:
            ...
            for magpol_item in self.magpol.list_magpol:
                try:
                    path = pathname_lhcb+'/efficiency/'+self.channel.name+"/"+self.source.name+"/"+year_item.name+"/"+magpol_item.name+'/preliminary_cuts_info.json'
                    print(path)
                    with open(path) as f:
    ...
    def get_final_cuts_eff(self):
        ...
        for year_item in self.year.list_years:
            ...
            for magpol_item in self.magpol.list_magpol:
                try:
                    path = pathname_lhcb+'/efficiency/'+self.channel.name+"/"+self.source.name+"/"+year_item.name+"/"+magpol_item.name+'/final_cuts_info.json'
                    with open(path) as f:
    ...
    def store_events(self, paths, list_var, cuts, Kspipi_ampModel, aliases = None):
        ...
        #Currently working with ampgen model?
        tmp_Amp    = Kspipi_ampModel.amp(
            (p1,p2,p3)
        )   
        tmp_Ampbar = Kspipi_ampModel.ampbar(
            (p1, p2, p3)
        )   
        # indices    = []
