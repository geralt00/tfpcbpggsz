...
void D0ToKspipi2018::init(){
  ...
  parameter_filename = "/software/pc24403/tfpcbpggsz/tfpcbpggsz/amp_masses/BELLE2018_data.txt";
...
std::vector<std::complex<double>> D0ToKspipi2018::get_amp(double _zm, double _zp) const
{
  ...
  const double mSq0p = _zp;//masses_from_rotvar(_zp,_zm)[0];
  const double mSq0n = _zm;//masses_from_rotvar(_zp,_zm)[1];
...
std::vector<std::vector<std::complex<double>>> D0ToKspipi2018::AMP(vector<double> _zm, vector<double> _zp) const{
  ...
  for(int i = 0; i < _zp.size(); i++){
    results.push_back(get_amp(_zp[i], _zm[i]));
  }
  return results;
}
