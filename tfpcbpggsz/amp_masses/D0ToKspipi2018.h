...
public:
  ...
  void init();  
  std::vector<std::complex<double>> get_amp(double _x, double _y) const;
  std::vector<std::vector<std::complex<double>>> AMP(vector<double> _x, vector<double> _y) const;
  // arrays for the Dalitz model results without systematics
  std::complex<double> Amp(const DalitzPoint& point, const double* ) const;
