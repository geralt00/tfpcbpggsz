...
std::vector < double > func_var_rotated(double sp, double sm, double zpmax, double zpmin, double zmmax, double zmmin) {
  double zp = (sp + sm)/2;
  double zm = (sp - sm)/2;
  double m1_ = 2.23407421671132946;
  double c1_ = -3.1171885586526695;
  double m2_ = 0.8051636393861085;
  double c2_ = -9.54231895051727e-05;


  double zp_p = m1_ * zp + c1_;
  double zm_p = m2_ * zm + c2_;
  std::vector< double > res = {zp_p, zm_p};
  return res;
}
...
std::vector < double > func_var_rotated_stretched(double sp, double sm, double zpmax, double zpmin, double zmmax, double zmmin) {
  ...
  double zp_p = m1_ * zp + c1_;
  double zm_p = m2_ * zm + c2_;
  double antiSym_scale = 2.0;
  double antiSym_offset = 2.0;
  double zm_pp = antiSym_scale * zm_p / (antiSym_offset + zp_p);
  std::vector< double > res = {zp_p, zm_pp};
  return res;
}
...
vector<double> p4_to_phsp(vector<double> ks, vector<double> pi1, vector<double> pi2)
{
  double s12 = pow(ks[0]+pi1[0],2) - pow(ks[1]+pi1[1],2) - pow(ks[2]+pi1[2],2) - pow(ks[3]+pi1[3],2);
  double s13 = pow(ks[0]+pi2[0],2) - pow(ks[1]+pi2[1],2) - pow(ks[2]+pi2[2],2) - pow(ks[3]+pi2[3],2);
