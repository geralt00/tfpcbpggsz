...
std::complex<double> Resonance::contribution(const DalitzPoint& point) const {
  ...
  const double x =  point.q("0p"); // // is either msquared01 for D0 or msquared02 for D0bar
  const double z =  point.q("0n"); // // is either msquared02 for D0 or msquared01 for D0bar
...
std::complex<double> Resonance::amplitude_KMatrix(double mab2) const {
	...
	double _s0prod = _Kmatrix_s_prod_0;
